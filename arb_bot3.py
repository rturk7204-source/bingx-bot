...
"""
Funding Rate Arbitrage Bot — LYN-USDT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ИЗОЛЯЦИЯ ОТ SMC-БОТА (bot.py):
  ✓ Символ:    LYN-USDT (не входит в SYMBOLS бота)
  ✓ Стейт:     arb_state3.json  (бот пишет trades.json)
  ✓ Лог:       arb_bot3.log     (бот пишет bot.log)
...
# ARB-бот торгует: LYN-USDT  ← нет пересечений
...
STATE_FILE = f"{BOT_DIR}/arb_state3.json"   # НЕ trades.json !
LOG_FILE   = f"{BOT_DIR}/arb_bot3.log"      # НЕ bot.log !
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [ARB3] %(levelname)s  %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
...
def cmd_status():
    ...
    print("=" * 58)
    print(f"  ARB3 BOT — {SYMBOL}  |  {datetime.now(timezone.utc).strftime('%H:%M UTC')}")
    print(f"  [SMC торгует BTC ETH SOL SUI DOGE ADA XRP LINK BNB HYPE]")
    print(f"  [ARB3 торгует: LYN-USDT ← клон #3]")
    print("=" * 58)
    ...
    if s.get("position_open"):
        ...
        print(f"  Спот LONG : {sq:.6f} LYN  PnL ${spnl:+.4f}")
...
def cmd_enter():
    ...
    import fcntl
    _lock_path = "/tmp/arb_bot3.enter.lock"
    ...
    log.info(f"Покупаем LYN: ${SPOT_BUDGET} USDT (symbol=LYN-USDT)...")
    ...
    log.info(f"   Спот LONG  : {spot_qty:.6f} LYN (${SPOT_BUDGET:.0f})")
    ...
    if rate < EXIT_RATE:
        ...
        if bad >= BAD_PERIODS:
            ...
            if netto_check < NETTO_EXIT_THRESHOLD:
                msg = f"🔴 LYN: bad={bad}/{BAD_PERIODS} И нетто ${netto_check:.2f} < ${NETTO_EXIT_THRESHOLD}. АВТОЗАКРЫТИЕ."
                log.error(msg)
                tg_send(msg)
                cmd_exit()
                return
            else:
                msg = f"⚠️ LYN: ставка ниже порога {BAD_PERIODS} периодов. Rate={rate*100:.4f}%, нетто ${netto_check:.2f} — держим, только алерт."
                tg_send(msg)
...
def cmd_exit():
    ...
    log.info("2/2 Продаём LYN на споте...")
    qty = get_spot_token()
    spot_ok = True
    if qty > 0:
        spot_ok = sell_spot(qty).get("code") == 0
    else:
        log.warning("LYN не найден на споте")
