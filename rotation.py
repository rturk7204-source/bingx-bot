...
def load_graveyard():
    ...
    for sym, rec in gy.items():
        ...
        if now - ejected_at < timedelta(hours=GRAVEYARD_COOLDOWN_H):
            active[sym] = rec
...
def add_to_graveyard(symbol, reason):
    gy = load_graveyard()
    gy[symbol] = {
...
def execute_rotation(decision, dry_run=True):
    ...
    if "109400" in combined_output or "API orders are temporarily disabled" in combined_output:
        ...
        add_to_graveyard(decision["new_symbol"], "api_vol_lock")
        lines.append(f"    🪦 {decision['new_symbol']} → graveyard ({GRAVEYARD_COOLDOWN_H}ч cool-down, tag=api_vol_lock)")
