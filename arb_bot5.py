...
"""
Funding Rate Arbitrage Bot — PTB-USDT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ИЗОЛЯЦИЯ ОТ SMC-БОТА (bot.py):
  ✓ Символ:    PTB-USDT (не входит в SYMBOLS бота)
  ✓ Стейт:     arb_state5.json  (бот пишет trades.json)
  ✓ Лог:       arb_bot5.log     (бот пишет bot.log)
...
# ARB-бот торгует: PTB-USDT  ← нет пересечений
...
STATE_FILE = f"{BOT_DIR}/arb_state5.json"   # НЕ trades.json !
LOG_FILE   = f"{BOT_DIR}/arb_bot5.log"      # НЕ bot.log !
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [ARB5] %(levelname)s  %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
...
def cmd_status():
    ...
    print("=" * 58)
    print(f"  ARB5 BOT — {SYMBOL}  |  {datetime.now(timezone.utc).strftime('%H:%M UTC')}")
    print(f"  [SMC торгует BTC ETH SOL SUI DOGE ADA XRP LINK BNB HYPE]")
    print(f"  [ARB5 торгует: {TOKEN}-USDT ← клон #5]")
    print("=" * 58)
...
def cmd_enter():
    ...
    import fcntl
    _lock_path = "/tmp/arb_bot5.enter.lock"
    ...
    log.info(f"Покупаем {TOKEN}: ${SPOT_BUDGET} USDT (symbol={TOKEN}-USDT)...")
    ...
    if rate < EXIT_RATE:
        ...
        if bad >= BAD_PERIODS:
            ...
            if netto_check < NETTO_EXIT_THRESHOLD:
                msg = f"🔴 {TOKEN}: bad={bad}/{BAD_PERIODS} И нетто ${netto_check:.2f} < ${NETTO_EXIT_THRESHOLD}. АВТОЗАКРЫТИЕ."
                log.error(msg)
                tg_send(msg)
                cmd_exit()
                return
            else:
                msg = f"⚠️ {TOKEN}: ставка ниже порога {BAD_PERIODS} периодов. Rate={rate*100:.4f}%, нетто ${netto_check:.2f} — держим, только алерт."
                tg_send(msg)
