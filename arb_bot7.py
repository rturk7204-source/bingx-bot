...
"""
Funding Rate Arbitrage Bot — Клон-7 (ловец свежих листингов)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Блок 8 (Часть 3): Седьмой бот для ловли свежих листингов:
  ✓ SYMBOL = placeholder, переопределяется rotation когда ловит новый листинг
  ✓ Стейт:     arb_state7.json (изоляция от ботов 1–6)
  ✓ Лог:       arb_bot7.log
  ✓ Архитектурно идентичен клонам 2–6 (все 4 команды)
  ✓ default_notional=100 (рисковые свежие листинги — меньший размер)

Команды:
  python3 arb_bot7.py            → статус
  python3 arb_bot7.py --enter    → открыть позицию
  python3 arb_bot7.py --monitor  → проверка (cron каждые 30 мин)
  python3 arb_bot7.py --exit     → закрыть всё
"""
...
LEVERAGE    = 3       # 3x → notional $120 = совпадает со спотом → delta=0
...
STATE_FILE = f"{BOT_DIR}/arb_state7.json"   # НЕ trades.json !
LOG_FILE   = f"{BOT_DIR}/arb_bot7.log"      # НЕ bot.log !
...
def cmd_status():
    ...
    if s.get("position_open"):
        ...
        print(f"  Спот LONG : {sq:.6f} {SYMBOL.split(chr(45))[0]}  PnL ${spnl:+.4f}")
...
def cmd_enter():
    ...
    import fcntl
    _lock_path = "/tmp/arb_bot7.enter.lock"
    ...
    log.info(f"Покупаем {SYMBOL.split(chr(45))[0]}: ${SPOT_BUDGET} USDT (symbol={SYMBOL})...")
    ...
    log.info(f"   Спот LONG  : {spot_qty:.6f} {SYMBOL.split(chr(45))[0]} (${SPOT_BUDGET:.0f})")
    ...
    if rate < EXIT_RATE:
        ...
        if bad >= BAD_PERIODS:
            ...
            if netto_check < NETTO_EXIT_THRESHOLD:
                msg = f"🔴 {SYMBOL}: bad={bad}/{BAD_PERIODS} И нетто ${netto_check:.2f} < ${NETTO_EXIT_THRESHOLD}. АВТОЗАКРЫТИЕ."
                log.error(msg)
                tg_send(msg)
                cmd_exit()
                return
            else:
                msg = f"⚠️ {SYMBOL}: ставка ниже порога {BAD_PERIODS} периодов. Rate={rate*100:.4f}%, нетто ${netto_check:.2f} — держим, только алерт."
                tg_send(msg)
...
def cmd_exit():
    ...
    log.info(f"2/2 Продаём {SYMBOL.split(chr(45))[0]} на споте...")
    qty = get_spot_token()
    spot_ok = True
    if qty > 0:
        spot_ok = sell_spot(qty).get("code") == 0
    else:
        log.warning(f"{SYMBOL.split(chr(45))[0]} не найден на споте")
