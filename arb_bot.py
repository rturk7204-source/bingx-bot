...
"""
Funding Rate Arbitrage Bot — RIVER-USDT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ИЗОЛЯЦИЯ ОТ SMC-БОТА (bot.py):
  ✓ Символ:    RIVER-USDT (не входит в SYMBOLS бота)
  ✓ Стейт:     arb_state.json  (бот пишет trades.json)
  ✓ Лог:       arb_bot.log     (бот пишет bot.log)
...
LEVERAGE    = 2       # 2x → notional $200 = совпадает со спотом → delta=0
...
STATE_FILE = f"{BOT_DIR}/arb_state.json"   # НЕ trades.json !
LOG_FILE   = f"{BOT_DIR}/arb_bot.log"      # НЕ bot.log !
...
def cmd_status():
    ...
    print("=" * 58)
    print(f"  ARB BOT — {SYMBOL}  |  {datetime.now(timezone.utc).strftime('%H:%M UTC')}")
    print(f"  [SMC торгует: BTC ETH SOL SUI DOGE ADA XRP LINK BNB HYPE]")
    print(f"  [ARB торгует: {TOKEN}-USDT ← нет пересечений]")
    print("=" * 58)
...
def cmd_enter():
    ...
    import fcntl
    _lock_path = "/tmp/arb_bot.enter.lock"
    ...
    log.info(f"Покупаем {TOKEN}: ${SPOT_BUDGET} USDT...")
...
def cmd_monitor():
    ...
    if rate < EXIT_RATE:
        ...
        if bad >= BAD_PERIODS:
            # Только алерт — закрытие RIVER только вручную или по критической марже
            msg = f"⚠️ {TOKEN}: ставка ниже порога {BAD_PERIODS} периодов подряд! Rate={rate*100:.4f}%. Проверь вручную."
            tg_send(msg)
            log.warning(f"Алерт отправлен. Автозакрытие по ставке ОТКЛЮЧЕНО.")
    elif bad > 0:
