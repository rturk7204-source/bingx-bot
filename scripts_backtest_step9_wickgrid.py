"""
Шаг 9: сетка по wick_ratio порогу.
База = step5_final. Один фильтр на разных порогах + LONG-only/SHORT-only/обе стороны.
"""
...
for sym,K in data.items():
    for i in range(100, len(K)-50, 4):
        ...
        cur=K[i]
        body=abs(cur['c']-cur['o']) or 0.0000001
        if sig['dir']=='LONG':
...
# распределение wick
ws=sorted([s['wick'] for s in setups])
print(f"  wick: min {ws[0]:.2f} med {ws[len(ws)//2]:.2f} p75 {ws[int(len(ws)*0.75)]:.2f} max {ws[-1]:.2f}\n")

def run(wick_max, side, label):
    trs=[]
    for s in setups:
        if side=='both':
            if s['wick']>wick_max: continue
        elif side=='long_only':
            if s['dir']=='LONG' and s['wick']>wick_max: continue
        elif side=='short_only':
            if s['dir']=='SHORT' and s['wick']>wick_max: continue
...
for t in THRESHOLDS:
    r=run(t, 'both', f'wick≤{t}')
    sd=r['sum_R']/abs(r['max_DD']) if r['max_DD'] else 0
    delta=r['sum_R']-prod['sum_R']
    print(f"  {f'≤{t}':>10s} | {r['n']:>4d} | {r['WR']:>5.1f} | {r['avg_R']:>+7.3f} | {r['sum_R']:>+8.1f} | {r['max_DD']:>7.1f} | {sd:>7.2f}  Δ={delta:+.1f}R")

print("\n=== STEP 9b: wick фильтр только для LONG ===")
print(f"{'threshold':>10s} | {'n':>4s} | {'WR%':>5s} | {'avg_R':>7s} | {'sum_R':>8s} | {'DD':>7s} | sum/|DD|")
print("-"*78)
for t in THRESHOLDS:
    r=run(t, 'long_only', f'L_wick≤{t}')
    sd=r['sum_R']/abs(r['max_DD']) if r['max_DD'] else 0
    delta=r['sum_R']-prod['sum_R']
    print(f"  {f'≤{t}':>10s} | {r['n']:>4d} | {r['WR']:>5.1f} | {r['avg_R']:>+7.3f} | {r['sum_R']:>+8.1f} | {r['max_DD']:>7.1f} | {sd:>7.2f}  Δ={delta:+.1f}R")

print("\n=== STEP 9c: wick фильтр только для SHORT ===")
print(f"{'threshold':>10s} | {'n':>4s} | {'WR%':>5s} | {'avg_R':>7s} | {'sum_R':>8s} | {'DD':>7s} | sum/|DD|")
print("-"*78)
for t in THRESHOLDS:
    r=run(t, 'short_only', f'S_wick≤{t}')
    sd=r['sum_R']/abs(r['max_DD']) if r['max_DD'] else 0
    delta=r['sum_R']-prod['sum_R']
    print(f"  {f'≤{t}':>10s} | {r['n']:>4d} | {r['WR']:>5.1f} | {r['avg_R']:>+7.3f} | {r['sum_R']:>+8.1f} | {r['max_DD']:>7.1f} | {sd:>7.2f}  Δ={delta:+.1f}R")
